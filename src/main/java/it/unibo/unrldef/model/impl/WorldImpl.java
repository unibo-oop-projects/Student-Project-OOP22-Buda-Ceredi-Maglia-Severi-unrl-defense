...
public class WorldImpl implements World{
    ...
    private final static double WARRANTY_MARGIN = 0.2;
        ...
        while (!end) {
            ...
            switch (dir.getFirst()) {
                case UP:
                    ...
                    ul = new Position(B.getX() - (PATH_DEPHT + WARRANTY_MARGIN)/2, B.getY());
                    dr = new Position(A.getX() + (PATH_DEPHT + WARRANTY_MARGIN)/2, A.getY());
                ...
                case DOWN:
                    A = pathCur;
                    B = new Position(A.getX(), A.getY() + dir.getSecond());
                    ul = new Position(A.getX() - (PATH_DEPHT + WARRANTY_MARGIN)/2, A.getY());
                    dr = new Position(B.getX() + (PATH_DEPHT + WARRANTY_MARGIN)/2, B.getY());
                    tmp = sorroundingEnemies.stream()
                            .filter(x -> this.enemiesInArea(ul, dr).contains(x))
                            .reduce((a, b) -> a.getPosition().get().getY() > b.getPosition().get().getY() ? a : b);
                    break;
                case RIGHT:
                    ...
                    ul = new Position(A.getX(), A.getY() - (PATH_DEPHT + WARRANTY_MARGIN)/2 );
                    dr = new Position(B.getX(), B.getY() + (PATH_DEPHT + WARRANTY_MARGIN)/2);
                    tmp = sorroundingEnemies.stream()
                            .filter(x -> this.enemiesInArea(ul, dr).contains(x))
                ...
                case LEFT:
                    A = pathCur;
                    B = new Position(A.getX() - dir.getSecond(), A.getY());
                    ul = new Position(B.getX(), B.getY() - (PATH_DEPHT + WARRANTY_MARGIN)/2);
                    dr = new Position(A.getX(), A.getY() + (PATH_DEPHT + WARRANTY_MARGIN)/2);
                    tmp = sorroundingEnemies.stream()
                            .filter(x -> this.enemiesInArea(ul, dr).contains(x))
                            .reduce((a, b) -> a.getPosition().get().getX() < b.getPosition().get().getX() ? a : b);
                    break;
